  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Cloudinary redentials not found" },
        { status: 500 }
      );
    }
    ...
    const result = await new Promise<CloudinaryUploadResukt>(
      (resolve, reject) => {
        const uploadStream = cloudinary.uploader.upload_stream(
          {
            resource_type: "video",
            folder: "vidoe-uploads",
            transformation: [{ quality: "auto", fetch_format: "mp4" }],
          },
          (error, result) => {
            if (error) reject(error);
            else resolve(result as CloudinaryUploadResukt);
          }
        );
